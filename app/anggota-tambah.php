<?php
	...
	if(!isset($_POST['simpan'])) {
		...
		$query2 = mysqli_query($db_conn, "SELECT max(idanggota) as id_anggota FROM tbanggota");
		$data = mysqli_fetch_array($query2);
		$id_anggota_number = $data['id_anggota'];

		$urutan = (int) substr($id_anggota_number, 2, 3);
		$urutan++;

		$id_anggota_number = 'AG' . sprintf("%03s", $urutan);
?>
<div id="container">
		...
		<form class="my-4" enctype="multipart/form-data" action="" method="post">
			<div class="form-row">
				<div class="form-group col-md-6">
					...
					<input type="text" class="form-control" name="id_anggota" value="<?php echo $id_anggota_number; ?>" readonly>
				</div>
				<div class="form-group col-md-6">
					<label for="name_lengkap">Nama Lengkap</label>
					<input type="text" class="form-control" name="nama_lengkap" placeholder="Masukan nama Anggota">
				</div>
			</div>
			...
			<div class="form-row">
				<div class="form-group col-md-6">
					<label for="inputCity">Upload Foto</label>
					<input type="file" name="foto" id="foto" class="form-control">
				</div>
				<div class="form-group col-md-6">
					<label for="alamat">Alamat</label>
					<textarea rows="3" cols="40" name="alamat" class="form-control" required></textarea>
				</div>
			</div>
			<div class="form-row">
