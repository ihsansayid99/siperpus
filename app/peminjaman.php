<?php
	...
	if(!isset($_POST['cari'])) { // Jika tidak melakukan pencarian anggota
		...
		buku INNER JOIN transaksi ON transaksi.id_buku = buku.id_buku INNER JOIN tbanggota ON transaksi.id_anggota = tbanggota.idanggota INNER JOIN admin ON transaksi.id_admin = admin.id_admin ORDER BY id_transaksi DESC LIMIT {$offset}, {$_QUERY_LIMIT};";
		...
		if(!empty($kata_kunci)) {
			...
			WHERE transaksi.id_transaksi LIKE '%{$kata_kunci}%' OR tbanggota.nama LIKE '%{$kata_kunci}%' OR buku.judul_buku LIKE '%{$kata_kunci}%' OR transaksi.tanggal_pinjam LIKE '%{$kata_kunci}%' OR transaksi.tanggal_kembali LIKE '%{$kata_kunci}%' OR admin.nm_admin LIKE '%{$kata_kunci}%' ORDER BY transaksi.id_transaksi DESC;";
			$query = mysqli_query($db_conn, $sql);
			$row = $query->num_rows;
		}
	}
			...
			<div class="page-content">
				<div class="table-upper">
					<div class="table-upper-left">
						<a href="index.php?p=peminjaman-tambah" class="btn btn-success btn-medium">Tambah</a>
						<a href="./app/peminjaman-cetak-daftar.php" title="Cetak Anggota" target="_blank">
				...
				?>
					<tbody>
						<tr>
							...
							<td><?php echo ($data['tanggal_kembali'] == NULL ? "<span class='badge badge-danger'>Belum Kembali</span>" : "<span class='badge badge-success'>Sudah Kembali</span>") ?></td>
							<td class="text-center">
								<a href="index.php?p=peminjaman-ubah&id=<?php echo $data['id_transaksi']; ?>" class="btn btn-warning btn-sm">Ubah</a>
							</td>
						</tr>
					</tbody>
				<?php
